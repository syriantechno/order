...
function math_vat_rate(vat) {
    var math_tax_rate = '';
    if (vat.length == 2) {
        math_tax_rate = parseFloat('1.' + vat);
    } else {
        math_tax_rate = parseFloat('1.' + '0' + vat);
    }
...
$(document).ready(function () {
    $('[data-function="add_div_item"]').click(function () {
        ...
        var max_element = $(this).attr('data-max_element');
        if (max_element == 'undefined') {
            max_element = '99';
        }
        var div_id = Math.floor(Math.random() * 1000) + 1;

        if ($(div_global + ' .clone_item').length >= max_element) {
            console.log('max div sayisina ulasildi.');
            return false;
        }
        {
