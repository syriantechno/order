...
function editor(param = {selector: "", plugins: "", toolbar: "", height: "500", menubar: false}) {
    ...
    if (param.selector == "" || param.selector == undefined) {
        param.selector = '#content';
    }
    if (param.menubar == "" || param.menubar == undefined) {
        param.menubar = false;
    }
...
function get_notification_count_query(param) {
    getXHR({
        'method': 'get',
        'url': get_site_url('admin/user/getNotification.php?query_type=count&session_id=' + window.session_id + '&box=' + param.box)
    }, function (data) {
        if (data == "empty") {
            var count = 0;
        } else {
            ...
            if (count != last_count) {
                if (count > parseInt(last_count)) {
                    ...
                    if (!is_mobile()) {
                        getXHR({
                            'method': 'get',
                            'url': get_site_url('admin/user/lastNotificationInfo.php?session_id=' + window.session_id)
...
function set_notification($this, object) {
    getXHR({
        'method': 'POST',
        'url': get_site_url('admin/user/setNotification.php?session_id=' + window.session_id + '&args=' + JSON.stringify(object))
    }, function (data) {
...
function get_notification(elem, box, open_control = true) {
    ...
    getXHR({
        'method': 'get',
        'url': get_site_url('admin/user/getNotification.php?query_type=list&session_id=' + window.session_id + '&box=' + box)
    }, function (data) {
        if (data != "empty" && data != false) {
            elem.innerHTML = data;
        }
...
function notification_all_readit_button($this, $list) {
    ...
    for (var i = elem.length - 1; i >= 0; i--) {
        set_notification($this, {
            'id': elem[i].className.match(/notification-(.*?) /gmi)[0].replace('notification-', '').trim(),
            'status': '1',
            'read_it': '1'
        })
    } // endfor
...
function chat_list(type) {
    if (!type || (type != "task" && type != "message")) {
        window.message_type = "message";
    } else {
        window.message_type = type;
    }
...
function send_message($this, type = "message") {
    ...
    if (!message) {
        var message = $this.querySelector('input[type="text"]');
    }
    ...
    if (message) {
        ...
        if (content.length >= 1) {
            ...
            getXHR({
                'method': 'POST',
                'url': get_site_url('admin/user/sendMessage.php?session_id=' + window.session_id + '&type=' + type),
                'send': form
            }, function (data) {
                if (data == 'true') {
...
function get_new_message(class_name = "", type = "message") {
    ...
    getXHR({
        'method': 'get',
        'url': get_site_url('admin/user/getMessage.php?type=' + get_type + '&query_message=' + chat_list.getAttribute('id') + '&last_view_message=' + window.get_last_message_id + '&get_message=new&session_id=' + window.session_id),
        timeout: true
    }, function (data) {
        clearInterval(window.getMessage);
                ...
                for (var i = matches.length - 1; i >= 0; i--) {
                    ...
                    if (!document.querySelector(".message-" + div.getAttribute('id'))) {
                        ...
                        if (chat_list.appendChild(div)) {
                            ...
                            setTimeout(function () {
                                if (chat_list.scrollTop != chat_list.scrollHeight) {
                                    chat_list.scrollTop = chat_list.scrollHeight;
                                }
                            }, 500);

                            if (class_name == 'get') {
                                document.title = "(1) " + chat_list.lastElementChild.getAttribute("title") + " - " + chat_list.lastElementChild.getAttribute("username");
                            }
...
function chat_list_scroll(chat_list, type) {
    chat_list.onscroll = function () {
        if (chat_list.scrollTop < 10) {
            ...
            getXHR({
                'method': 'get',
                'url': get_site_url('admin/user/getMessage.php?type=' + old_type + '&query_message=' + chat_list.getAttribute("id") + '&get_message=old&list_old_message=' + window.get_first_message_id + '&session_id=' + window.session_id)
            }, function (data) {
                if (data != "empty" && data != false) {
                    if (matches = data.match(/<div class="message-elem *?([\s\S]*?)<\/div><!--\/ \.message-elem \/-->/gmi)) {
                        for (var i = matches.length - 1; i >= 0; i--) {
                            setTimeout(function () {
                                parent(chat_list, '.chat-container').classList.remove("loader");
                            }, 500);
                            ...
                            if (!document.querySelector(".message-" + div.getAttribute('id'))) {
                                if (i == 0) {
                                    window.get_first_message_id = div.getAttribute('id');
                                }
...
function til_dynamic_tab($this, $elem) {
    ...
    for (var i = 0; i < all_tabs.length; i++) {
        if ($elem != all_tabs[i]) all_tabs[i].classList.remove('open');
    }

    var all_btn = document.querySelectorAll('[dynamic-tab-btn]');
    for (var i = 0; i < all_btn.length; i++) {
        if ($this != all_btn[i]) all_btn[i].classList.remove('active');
    }
...
function clear_writing(obj) {
    getXHR({
        'method': 'get',
        'url': get_site_url('admin/user/message-writing.php?top_id=' + window.set_writing.top_id + '&clear_writing=true&session_id=' + window.session_id)
    }, function (data) {
        if (data != 'empty' && data != false) {
            if (data == 'true') {
                return true;
...
function set_writing(obj) {
    if (obj) {
        getXHR({
            'method': 'get',
            'url': get_site_url('admin/user/message-writing.php?top_id=' + window.set_writing.top_id + '&set_value=' + obj.set_value + '&session_id=' + window.session_id)
        }, function (data) {
            if (data != 'empty' && data != false) {
                if (data == 'true') {
...
function get_writing(selector) {
    setInterval(function () {
        getXHR({
            'method': 'get',
            'url': get_site_url('admin/user/message-writing.php?top_id=' + window.set_writing.top_id + '&&get=true&session_id=' + window.session_id)
        }, function (data) {
            if (data != 'empty') {
