        ...
        if (!is_alert(__FUNCTION__)) { // eger herhangi bir hata yok ise
            ...
            if (db()->query("INSERT INTO " . dbname('users') . " " . sql_insert_string($insert) . " ")) {
                if ($insert_id = db()->insert_id) {
                    if ($args['add_alert']) {
                        add_alert('تم إنشاء حساب المستخدم.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('table_id' => 'users:' . get_active_user_id(), 'log_key' => 'add_user', 'log_text' => 'تم إنشاء حساب مستخدم جديد.' . _b($insert['username']) . ''));
                    }
                    return $insert_id;
                } else {
                    return false;
...
function update_user($user_id, $args)
{
    ...
    if (!have_log()) { // bu islem daha onceden gercekles mi?
        ...
        if (!is_alert(__FUNCTION__)) { // eger herhangi bir hata yok ise
            ...
            // string transactions
            if (isset($update['username'])) {
                $update['username'] = til_get_strtolower($update['username']);
            }
            if (isset($update['name'])) {
                $update['name'] = til_get_strtoupper($update['name']);
            }
            if (isset($update['surname'])) {
                $update['surname'] = til_get_strtoupper($update['surname']);
            }
            ...
            if (db()->query("UPDATE " . dbname('users') . " SET " . sql_update_string($update) . " WHERE id='" . $user_id . "' ")) {
                if (db()->affected_rows > 0) {
                    ...
                    if ($args['add_alert']) {
                        add_alert('تم تحديث حساب المستخدم.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('table_id' => 'users:' . $user_id, 'log_key' => 'update_user', 'log_text' => _b($old_user->display_name) . ' تم تحديث حساب المستخدم.', 'meta' => log_compare($old_user, $new_user)));
                    }
                    return true;
                } else {
                    return false;
    ...
    if (isset(til()->user[$where['id']]) AND $_til) {
        ...
        if ($query = db()->query("SELECT * FROM " . dbname('users') . " " . sql_where_string($where) . " ")) {
            if ($query->num_rows) {
                ...
                if (!$return->avatar) {
                    $return->avatar = get_template_url('img/no-avatar-' . $return->gender . '.jpg');
                } else {
                    $return->avatar = get_site_url($return->avatar);
                }
                if (!isset($return->display_name)) {
                    $return->display_name = $return->username . ' ' . $return->surname;
                }
                til()->user[$return->id] = $return;
                return $return;
            } else {
                if ($args['add_alert']) {
                    add_alert('لم يتم العثور على حساب مستخدم يطابق المعايير التي تبحث عنها.', 'warning', __FUNCTION__);
                }
                return false;
            }
        } else {
...
function add_user_meta($user_id, $meta_key, $meta_value, $args = array())
{
    ...
    if (!$user = get_user($user_id)) {
        add_console_log('#' . $user_id . ' لم يتم العثور على المستخدم.', __FUNCTION__);
        return false;
    }
    if (empty($meta_key)) {
        add_console_log('لا يمكن أن يكون المفتاح ميتا فارغا.', __FUNCTION__);
        return false;
    }


    if (is_array($meta_value) or is_object($meta_value)) {
        $meta_value = json_encode_utf8($meta_value);
    }
...
function set_staff_salary($user_id)
{
    ...
    if (!$user = get_user($user_id)) {
        add_console_log('üye id bulunamadı', __FUNCTION__);
        return false;
    }
    ...
    if (!@$user_meta['date_start_work']) {
        add_console_log('لم يتم العثور على تاريخ بدء العمل.', __FUNCTION__);
        return false;
    }
    ...
    if ($user_meta['date_end_work']) {
        $calc_end_date = $user_meta['date_end_work'];
    } else {
        $calc_end_date = date('Y-m-d');
    }
    if (strtotime($calc_end_date) > time()) {
        $calc_end_date = date('Y-m-d');
    }
...
function get_user_access($val, $key, $prefix = '', $opt = array())
{
    ...
    // options required
    if (!is_array($opt)) {
        $opt = array('get_user_access' => $opt);
    }
    if (!isset($opt['get_user_access'])) {
        $opt['get_user_access'] = true;
    } // get_user_access() fonksiyonu calissin
