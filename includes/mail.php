...
function send_mail($args = array())
{
    ...
    if (!is_alert(__FUNCTION__)) {
        ...
        if (empty($_company)) {
            $_company = (object)array('name' => '', 'address' => '', 'district' => '', 'city' => '', 'country' => '', 'email' => '', 'phone' => '', 'gsm' => '');
        }


        // is email empty
        if (empty($option->send_address)) {
            if (!empty($_company->email)) {
                $option->send_address = $_company->email;
            }
        }
        ...
        if (!empty($option->send_address)) {
            ...
            add_alert('Mail göndermek için gerekli bilgiler bulunmadı.', 'danger', __FUNCTION__);
        }
    } // is_alert()
} //.send_mail()
