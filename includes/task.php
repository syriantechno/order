...
function add_task($rec_u_id, $args = array())
{
    ...
    if (!$rec_user = get_user($rec_u_id)) {
        add_alert('Görev atanacak kullanıcı bulunamadı.', 'danger', __FUNCTION__);
    }
    ...
    if (!have_log()) {
        if (!is_alert(__FUNCTION__)) {
            ...
            if (empty($choice)) {
                ...
                $insert['choice'] = json_encode_utf8($choice);
            }


            # eger mesaj basligi yok ise otomatik olustur
            if (!isset($insert['title'])) {
                $insert['title'] = preg_replace("/[^\d\pL\p{Zs}]+/u", "", mb_substr(strip_tags(html_entity_decode($insert['message'])), 0, 50, 'utf-8'));
            }
            if (!strlen($insert['title'])) {
                $insert['title'] = 'Konu yok';
            }
            ...
            if ($q_insert = db()->query("INSERT INTO " . dbname('messages') . " " . sql_insert_string($insert) . " ")) {
                if (db()->insert_id) {
                    ...
                    if ($args['add_alert']) {
                        add_alert('Mesaj gönderildi', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('table_id' => 'messages:' . $insert_id, 'log_key' => __FUNCTION__, 'log_text' => 'تم إرسال رسالة جديدة. ' . _b(get_user_info($rec_user->id, 'name') . ' ' . get_user_info($rec_user->id, 'surname'))));
                    }
                    return $insert_id;
                } else {
                    return false;
    ...
    if (!have_log(@$args['uniquetime'])) {
        if (!is_alert(__FUNCTION__, 'danger')) {
            ...
            if ($q_update = db()->query("UPDATE " . dbname("messages") . " SET " . sql_update_string($update) . " WHERE id='" . $task_id . "' ")) {
                if (db()->affected_rows) {
                    if ($args['add_alert']) {
                        add_alert('تم تحديث معلومات المهمة.', 'success', __FUNCTION__);
                    }
                    return true;
                } else {
                    return false;
...
function add_task_message($task_id, $args)
{
    ...
    # task_id var mi?
    if (!$task = get_task($task_id)) {
        add_alert('Görev ID bulunamadı.', 'danger', __FUNCTION__);
    }
    ...
    if (!have_log(@$args['uniquetime'])) {
        if (!is_alert(__FUNCTION__)) {
            ...
            $insert['date_update'] = date('Y-m-d H:i:s');
            $insert['sen_u_id'] = get_active_user('id');
            if (get_active_user('id') == $task->sen_u_id) {
                $insert['rec_u_id'] = $task->rec_u_id;
            } else {
                $insert['rec_u_id'] = $task->sen_u_id;
            }
            ...
            if ($q_insert = db()->query("INSERT INTO " . dbname('messages') . " " . sql_insert_string($insert) . " ")) {
                if (db()->insert_id) {
                    $insert_id = db()->insert_id;
                    if ($args['add_alert']) {
                        add_alert('وأضاف رسالة الرد على المهمة.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('table_id' => 'messages:' . $task_id, 'log_key' => __FUNCTION__, 'log_text' => 'تم إرسال رسالة الرد للمهمة. ' . _b('ID: ' . $task->id)));
                    }
                    ...
                    if (get_active_user('id') == $task->sen_u_id) {
                        $_task['update']['inbox_u_id'] = $task->rec_u_id;
                    } else {
                        $_task['update']['inbox_u_id'] = $task->sen_u_id;
                    }
