    ...
    if (!is_alert(__FUNCTION__) and !have_log(@$args['uniquetime'])) {
        ...
        if (move_uploaded_file($file['tmp_name'], $file_to_upload)) {
            if (isset($args['sizing'])) {
                ...
                if (!isset($args['sizing']['width'])) {
                    $args['sizing']['width'] = '150';
                }
                if (!isset($args['sizing']['height'])) {
            ...
            if ($args['add_alert']) {
                add_alert('تم تحميل ملف الصورة بنجاح.', 'success', __FUNCTION__);
            }
            if ($args['add_log']) {
                add_log(array('uniquetime' => @$args['uniquetime'], 'table_id' => 'users:' . get_active_user('id'), 'log_key' => 'image_upload', 'log_text' => 'Bir resim dosyası yükledi.'));
            }
...
function delete_image($file)
{
    ...
    if (file_exists($path)) { // dosya var mi?
        if (is_file($path)) { // dosya bir dosya mi?
            ...
            add_console_log('Silinecek öğe bir dosya değil.', __FUNCTION__);
            return false;
        }
    } else {
        add_console_log('Resim dosyası bulunamadı', __FUNCTION__);
        return false;
    }
} //.delete_image()
