...
function add_option($option_name, $args = array())
{
    ...
    if (!empty($option_name)) {
        ...
        add_alert('Option name Bulunamadı!');
    }
} //.add_option()
        ...
        if (!is_alert(__FUNCTION__)) {
            if ($q_insert = db()->query("INSERT INTO " . dbname('extra') . " " . sql_insert_string($insert) . " ")) {
                if (db()->insert_id) {
                    ...
                    if ($args['add_alert']) {
                        add_alert('Ekstra eklendi.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('uniquetime' => @$args['uniquetime'], 'table_id' => 'extra:' . $insert_id, 'log_key' => __FUNCTION__, 'log_text' => $insert['name'] . ' ekstra eklendi.'));
                    }
                    return $insert_id;
                } else {
                    return false;
        ...
        if (!is_alert(__FUNCTION__)) {
            if ($extra = get_extra($where)) {
                if ($q_update = db()->query("UPDATE " . dbname('extra') . " SET " . sql_update_string($update) . " " . sql_where_string($where) . " ")) {
                    ...
                    if ($args['add_alert']) {
                        add_alert('Ekstra güncellendi.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('uniquetime' => @$args['uniquetime'], 'table_id' => 'extra:' . $extra->id, 'log_key' => __FUNCTION__, 'log_text' => 'Eksra güncellendi.', 'meta' => log_compare($extra, $new_extra)));
                    }
                    return $extra->id;
                } else {
                    add_mysqli_error_log(__FUNCTION__);
...
function get_extra($args = array())
{
    ...
    if (!is_array($args)) {
        if (is_numeric($args)) {
            ...
            $args = array('where' => array('taxonomy' => $args));
        }
    }
