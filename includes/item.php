    ...
    if (!have_log()) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
        ...
        if (!is_alert('add_item', 'danger')) { // eger herhangi bir hata yok ise
            ...
            if ($q_is_code->num_rows) { // eger veritabanında ayni barkod var ise
                ...
                if ($args['add_alert']) {
                    add_alert('<b>' . $insert['code'] . '</b> تم العثور على رمز المنتج على بطاقة منتج آخر. يرجى تغيير رمز المنتج أو <a href="' . get_site_url('admin/item/detail.php?id=' . $q_is_code->id) . '" target="_blank"><b>هنا</b></a> استخدام بطاقة المنتج.', 'warning', __FUNCTION__);
                }
                if ($q_is_code->status == '1') {
                    if ($args['add_alert']) {
                        add_alert('<u> أيضا، تم العثور على هذا الرمز بين بطاقات المنتج المحذوفة</u>. بطاقة المنتج <a href="' . get_site_url('admin/item/detail.php?id=' . $q_is_code->id) . '"><b>من هنا</b></a> يمكنك تفعيله مرة أخرى.', 'warning', __FUNCTION__);
                    }
                }
                ...
                if (db()->query("INSERT INTO " . dbname('items') . " " . sql_insert_string($insert) . " ")) {
                    ...
                    if ($args['add_alert']) {
                        add_alert('تمت اضافة بطاقة المادة بنجاح.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('uniquetime' => @$insert['uniquetime'], 'table_id' => 'items:' . $insert_id, 'log_key' => __FUNCTION__, 'log_text' => 'تمت إضافة بطاقة المنتج.'));
                    }
                    return $insert_id;
                } else {
...
function get_item($args, $_til = true)
{
    ...
    if ($query = db()->query("SELECT * FROM " . dbname('items') . " " . sql_where_string($where) . " ")) {
        if ($query->num_rows) {
            ...
            if ($args['add_alert']) {
                add_alert('لم يتم العثور على بطاقة منتج للمعايير التي كنت تبحث عنها.', 'warning', __FUNCTION__);
            }
            return false;
        }
    } else {
...
function update_item($where, $args)
{
    ...
    if (!isset($update['name'])) {
        $update['name'] = $old_item->name;
    }
    if (!isset($update['code'])) {
        $update['code'] = $old_item->code;
    }
    if (!isset($update['vat'])) {
        $update['vat'] = $old_item->vat;
    }
    if (!isset($update['p_purc'])) {
        $update['p_purc'] = $old_item->vat;
    }
    if (!isset($update['p_sale'])) {
        $update['p_sale'] = $old_item->vat;
    }
    ...
    if (!have_log()) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
        ...
        if (!is_alert(__FUNCTION__)) {
            ...
            if (!isset($update['type'])) {
                ...
                $update['type'] = input_check($update['type']);
            }
            ...
            if (isset($update['p_purc'])) {
                ...
                if (!isset($update['p_purc_out_vat'])) {
                    $update['p_purc_out_vat'] = $update['p_purc'] / get_set_vat($update['vat']);
                }
            ...
            if (isset($update['p_sale'])) {
                ...
                if (!isset($update['p_sale_out_vat'])) {
                    $update['p_sale_out_vat'] = $update['p_sale'] / get_set_vat($update['vat']);
                }
            ...
            if (!is_alert(__FUNCTION__, 'danger')) { // eger herhangi bir hata yok ise
                ...
                if ($q_is_code->num_rows) { // eger veritabanında ayni barkod var ise
                    ...
                    if ($args['add_alert']) {
                        add_alert('<b>' . $update['code'] . '</b> تم العثور على رمز المنتج على بطاقة منتج أخرى. يرجى تغيير رمز المنتج أو <a href="' . get_site_url('admin/item/detail.php?id=' . $q_is_code->id) . '" target="_blank"><b>هنا</b></a> hesabı kullanın.', 'warning', __FUNCTION__);
                    }
                    ...
                    if (db()->query("UPDATE " . dbname('items') . " SET " . sql_update_string($update) . " WHERE id='" . $old_item->id . "' ")) {
                        if (db()->affected_rows > 0) {
                            ...
                            if ($args['add_alert']) {
                                add_alert('تم تحديث بطاقة المادة.', 'success', __FUNCTION__);
                            }
                            if ($args['add_log']) {
                                add_log(array('uniquetime' => @$update['uniquetime'], 'table_id' => 'items:' . $old_item->id, 'log_key' => __FUNCTION__, 'log_text' => 'Ürün kartı güncellendi.', 'meta' => log_compare($old_item, $new_item)));
                            }
                            return true;
                        } else {
                            return false;
