...
function add_message($rec_u_id, $args = array())
{
    ...
    if (!$rec_user = get_user($rec_u_id)) {
        add_alert('Mesaj gönderilecek kullanıcı bulunamadı.', 'danger', __FUNCTION__);
    }
        ...
        if (!is_alert(__FUNCTION__)) {
            ...
            if ($q_insert = db()->query("INSERT INTO " . dbname('messages') . " " . sql_insert_string($insert) . " ")) {
                ...
                if (db()->insert_id) {
                    ...
                    if ($args['add_alert']) {
                        add_alert('<i class="fa fa-paper-plane-o"></i> تم إرسال الرسالة.', 'success', __FUNCTION__);
                    }
                    if ($args['add_log']) {
                    ...
                    if ($insert['top_id']) {
                        ...
                        if (db()->query("UPDATE " . dbname('messages') . " SET " . sql_update_string($real_message) . " WHERE id='" . $insert['top_id'] . "' ")) {
                        } else {
