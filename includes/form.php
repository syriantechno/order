    ...
    if (!have_log()) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
        ...
        if (!is_alert(__FUNCTION__)) {
            ...
            if (isset($set['new_account'])) {
                ...
                if (empty($_form['account_gsm'])) {
                    $_form['account_gsm'] = '5300000000';
                }
                ...
                if ($account_id = add_account($_account)) {
                    ...
                    add_alert('حدث الخطأ أثناء إنشاء بطاقة الحساب للنموذج.', 'danger', __FUNCTION__);
                    return false;
                }
            }
            ...
            if (!empty($_form['account_code'])) {
                if ($account = get_account(array('code' => $_form['account_code']))) {
                    ...
                    add_alert('"<b>' . $_form['account_code'] . '</b>" لم يتم العثور على رمز الحساب.', 'danger', __FUNCTION__);
                    return false;
                }
            }
        ...
        if ($old_form = get_form($id)) {
            if ($q_update = db()->query("UPDATE " . dbname('forms') . " SET " . sql_update_string($update) . " WHERE id='" . $id . "'")) {
                if (db()->affected_rows) {
                    ...
                    if ($args['add_log']) {
                        add_log(array('uniquetime' => @$args['uniquetime'], 'table_id' => 'forms:' . $id, 'log_key' => __FUNCTION__, 'log_text' => 'تم تحديث النموذج.', 'meta' => log_compare($old_form, $new_form)));
                    }
                    return true;
                } else {
                    return false;
...
function insert_form_item($args)
{
    ...
    if (!isset($args['uniquetime'])) {
        $args['uniquetime'] = '';
    }
    if (!isset($insert['form_id'])) {
        add_alert('خطأ في المعلمة: لا يمكن أن يكون معرف النموذج فارغا.');
        return false;
    }
    ...
    if (!have_log($args['uniquetime'])) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
        ...
        if (!$form = get_form($insert['form_id'])) {
            add_alert(_b($insert['form_id']) . ' لم يتم العثور على رقم النموذج.', 'warning', __FUNCTION__);
            return false;
        }
    ...
    if (!have_log($args['uniquetime'])) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
        ...
        if (!empty($args['item_code'])) {
            if (!$item = get_item(array('code' => $args['item_code']))) {
                ...
                if ($args['price'] < 0.01) {
                    if ($form->in_out == '0') {
                        $args['price'] = $item->p_purc;
                    } else {
                        $args['price'] = $item->p_sale;
                    }
                }
                if ($args['vat'] < 1) {
                    $args['vat'] = $item->vat;
        ...
        if (!is_alert(__FUNCTION__)) {
            ...
            if ($q_insert = db()->query("INSERT INTO " . dbname('form_items') . " " . sql_insert_string($args) . " ")) {
                if (db()->affected_rows) {
                    ...
                    if (isset($args['item_id']) and $args['item_id'] > 0) {
                        calc_item($args['item_id']);
                    }
...
function get_form_items($form_id, $opt = array())
{
    ...
    if (!isset($opt['status'])) {
        ...
        $opt['status'] = input_check($opt['status']);
    }
...
function add_form_meta($form_id, $meta_key, $meta_value)
{
    ...
    if (!is_alert(__FUNCTION__ . '-' . $alert_rnd)) {
        ...
        if ($form = get_form($form_id)) {
            ...
            add_console_log('Form bulunamadı', 'warning', __FUNCTION__);
            return false;
        }
...
function delete_form_meta($form_id, $args = array())
{
    ...
    if (empty($args)) {
        ...
        if ($q_delete = db()->query("DELETE FROM " . dbname('form_meta') . " WHERE id='" . $meta_id . "' ")) {
            if (db()->affected_rows) {
                if ($args['add_alert']) {
                    add_alert('Form ek bilgisi silindi.', 'warning', __FUNCTION__);
                }
            }
            return true;
        } else {
    ...
    if ($form = get_form($form_id)) {
        ...
        if ($q_delete = db()->query("DELETE FROM " . dbname('form_meta') . " " . sql_where_string($where) . " ")) {
            if (db()->affected_rows) {
                if ($args['add_alert']) {
                    add_alert('تم حذف النموذج معلومات إضافية.', 'warning', __FUNCTION__);
                }
            }
            return true;
        } else {
        ...
        if ($args['add_alert']) {
            add_alert('لم يتم العثور على النموذج', 'warning', __FUNCTION__);
        }
        return false;
    }
} //.delete_form_meta()
    ...
    if (!have_log(@$args['uniquetime'])) {
        ...
        if (!is_alert(__FUNCTION__)) {
            ...
            if ($extra_id = add_extra($_insert)) {
                ...
                if ($args['add_alert']) {
                    add_alert(_b($fs_cat_name) . ' إلى ' . _b($_insert['insert']['name']) . ' تمت إضافة حالة النموذج.', "success", __FUNCTION__);
                }
                if ($args['add_log']) {
                    add_log(array('uniquetime' => @$args['uniquetime'], 'table_id' => 'extra:' . $extra_id, 'log_key' => __FUNCTION__, 'log_text' => _b($fs_cat_name) . ' إلى ' . _b($_insert['insert']['name']) . ' تمت إضافة حالة النموذج.'));
                }
                return $extra_id;
            } else {
                return false;
        ...
        if ($old_status = get_form_status($_where)) {
            ...
            if ($extra_id = update_extra($_where, $_args)) {
                if ($extra = get_form_status($extra_id)) {
                    ...
                    if ($args['add_alert']) {
                        add_alert(_b($fs_cat_name) . ' için ' . _b($extra->name) . '</b>" تم تحديث حالة النموذج.', "success", __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('uniquetime' => @$args['uniquetime'], 'table_id' => 'extra:' . $extra->id, 'log_key' => __FUNCTION__, 'log_text' => _b($fs_cat_name) . ' için ' . _b($extra->name) . ' تم تحديث حالة النموذج.'));
                    }
                    return $extra->id;
                }
            } else {
            ...
            if ($args['add_alert']) {
                add_alert('لا يمكن العثور على عنصر حالة النموذج المراد تحديثه.', "warning", __FUNCTION__);
            }
            return false;
        }
...
function get_form_status_all($args = array())
{
    ...
    if (!is_array($args)) {
        if ($args == 'in' or $args == '0') {
            $args = array('in_out' => '0');
        } elseif ($args == 'out' or $args == '1') {
            $args = array('in_out' => '1');
        }
    }
...
function delete_form_status($delete_status_id, $new_status_id, $args = array())
{
    ...
    if (delete_extra($_args)) {
        ...
        if ($new_status = get_form_status($new_status_id)) {
            if (db()->query("UPDATE " . dbname('forms') . " SET status_id='" . $new_status_id . "' WHERE status_id='" . $delete_status_id . "' ")) {
                if (db()->affected_rows) {
                    $fs_cat_name = til()->form_status[$delete_status->taxonomy]['name'];
                    if ($args['add_alert']) {
                        add_alert(_b($fs_cat_name) . ' إلى ' . _b($delete_status->name) . '</b>" جميع أشكال وضع النموذج ' . $new_status->name . ' تم استبداله.', "success", __FUNCTION__);
                    }
                    if ($args['add_log']) {
                        add_log(array('table_id' => 'extra:' . $extra->id, 'log_key' => __FUNCTION__, 'log_text' => _b($fs_cat_name) . ' إلى ' . _b($delete_status->name) . '</b>" جميع أشكال وضع النموذج ' . $new_status->name . ' تم استبداله.'));
                    }
                    return true;
                } else {
                    return false;
        ...
        if ($args['add_alert']) {
            add_alert(_b($fs_cat_name) . ' إلى ' . _b($delete_status->name) . '</b>" تم حذف حالة النموذج.', "success", __FUNCTION__);
        }
        if ($args['add_log']) {
            add_log(array('table_id' => 'extra:' . $delete_status->id, 'log_key' => __FUNCTION__, 'log_text' => _b($fs_cat_name) . ' إلى ' . _b($delete_status->name) . ' تم حذف حالة النموذج.'));
        }
        return true;
    } else {
        return false;
...
function _set_form_status_default($_taxonomy)
{
    ...
    if (substr($_taxonomy, 0, 7) != 'til_fs_') {
        $_taxonomy = 'til_fs_' . $_taxonomy;
    }
...
function _set_form_status_args($args, $return_args = false)
{
    ...
    if (isset($args['in_out'])) {
        $_arr['val_enum'] = $args['in_out'];
    }
    if (substr($_arr['taxonomy'], 0, 7) != 'til_fs_') {
        $_arr['taxonomy'] = 'til_fs_' . $_arr['taxonomy'];
    }


    if (isset($_arr['name'])) {
        $_arr['name'] = til_get_ucwords($_arr['name']);
    }
...
function register_form_status($args)
{
    ...
    $_args = array();
    if (isset($args['taxonomy'])) {
        $_args['taxonomy'] = 'til_fs_' . remove_accents($args['taxonomy']);
    } else {
        return false;
    }
    if (isset($args['name'])) {
    ...
    if (isset($args['description']) and !empty($args['description'])) {
        $_args['description'] = $args['description'];
    } else {
        $_args['description'] = $_args['name'] . ' form durumu yönetimi';
    }
    if (isset($args['in_out'])) {
        $_args['in_out'] = $args['in_out'];
    } else {
        $_args['in_out'] = true;
    }
    if (isset($args['sms_template'])) {
        $_args['sms_template'] = $args['sms_template'];
    } else {
        $_args['sms_template'] = true;
    }
    if (isset($args['email_template'])) {
        $_args['email_template'] = $args['email_template'];
    } else {
        $_args['email_template'] = true;
    }
    if (isset($args['color'])) {
