...
function add_log($arr = array())
{
    ...
    if (!isset($arr['uniquetime'])) {
        if (isset($_POST['uniquetime'])) {
            $arr['uniquetime'] = $_POST['uniquetime'];
        } elseif (isset($_GET['uniquetime'])) {
            $arr['uniquetime'] = $_GET['uniquetime'];
        } else {
            $arr['uniquetime'] = get_uniquetime();
        }
    }
    if (!isset($arr['table_id'])) {
        $arr['table_id'] = '';
    }
    if (!isset($arr['user_id'])) {
        $arr['user_id'] = get_active_user_id();
    }
    if (!isset($arr['log_url'])) {
        $arr['log_url'] = $_SERVER['REQUEST_URI'];
    }
    if (!isset($arr['log_key'])) {
        $arr['log_key'] = '';
    }
    if (!isset($arr['log_text'])) {
        $arr['log_text'] = '';
    }
