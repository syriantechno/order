...
function sql_insert_string($arr = array(), $opt = array())
{
    ...
    if (!is_array($opt)) {
        $opt = array('return' => 'string');
    }
    if (!isset($opt['uniquetime'])) {
        unset($arr['insert']['uniquetime']);
    }
    if (!isset($opt['add'])) {
        unset($arr['insert']['add']);
    }
    if (!isset($opt['update'])) {
        unset($arr['insert']['update']);
    }
    if (!isset($opt['delete'])) {
        unset($arr['insert']['delete']);
    }
    if (!isset($opt['again'])) {
        unset($arr['insert']['again']);
    }
    if (!isset($opt['id'])) {
        unset($arr['insert']['id']);
    }
...
function sql_where_string($arr)
{
        ...
        foreach ($arr as $key => $val) {
            if (is_numeric($key) and is_array($val)) {
                foreach ($val as $key1 => $val1) {
                    if (til_get_strtoupper($key1) == 'ORDER_BY') {
                        $order_by = $val1;
                    } elseif (til_get_strtoupper($key1) == 'LIMIT') {
                        $limit = $val1;
                    } else {
                        $r .= _where_string($key1, $val1, $arr);
                    }
                }
            } else {
                if (til_get_strtoupper($key) == 'ORDER_BY') {
                    $order_by = $val;
                } elseif (til_get_strtoupper($key) == 'LIMIT') {
                    $limit = $val;
                } else {
                    $r .= _where_string($key, $val, $arr);
                }
            }
        }
    }
    ...
    if ($order_by) {
        $r .= _where_string('order_by', $order_by);
    }
    if ($limit) {
        $r .= _where_string('limit', $limit);
    }
...
function old_sql_where_string($arr = array(), $opt = array())
{
    ...
    if (isset($arr['orderby'])) {
        $return .= ' ORDER BY ' . $arr['orderby'];
    }
    if (isset($arr['limit'])) {
        $return .= ' LIMIT ' . $arr['limit'];
    }
...
function db_query_list_return_user($query, $arr = array())
{
    ...
    while ($list = $query->fetch_object()) {
        if (isset($arr['prefix'])) {
            if (!$list->avatar) {
                $list->avatar = get_template_url('img/no-avatar-' . $list->gender . '.jpg');
            } else {
                $list->avatar = get_site_url($list->avatar);
            }
            if (!isset($list->display_name)) {
                $list->display_name = $list->name . ' ' . $list->surname;
            }
...
function get_ARR_helper_limit_AND_orderby($arr)
{
    ...
    if (@$arr['_GET'] == true) { // eger GET ile gelecek parametreler aktif ise GET parametlerine gore sorgu yapılabilir
        if (isset($_GET['page'])) {
            $arr['page'] = input_check($_GET['page']);
        }
        if (isset($_GET['limit'])) {
            if ($_GET['limit'] == 'false') {
                $arr['limit'] = false;
    ...
    if (!isset($arr['limit'])) {
        $arr['limit'] = til()->pg->list_limit;
    } else {
        $arr['limit'] = input_check($arr['limit']);
    }
    if (!isset($arr['page'])) {
        $arr['page'] = 0;
    } else {
        $arr['page'] = input_check($arr['page']);
    }
    if ($arr['page'] > 0) {
        $arr['page'] = ($arr['page'] * $arr['limit']) - $arr['limit'];
    }
    ...
    if (@$arr['_GET'] == true) { // eger GET ile gelecek parametreler aktif ise GET parametlerine gore sorgu yapılabilir
        if (isset($_GET['s'])) {
            $arr['s'] = input_check($_GET['s']);
        }
        if (isset($_GET['db-s-where'])) {
            $arr['db-s-where'] = input_check($_GET['db-s-where']);
        } else {
            $arr['db-s-where'] = 'all';
        }
        ...
        if (isset($_GET['status_id'])) {
            $arr['status_id'] = $_GET['status_id'];
        }
    }

    // input check
    if (isset($arr['s'])) {
        $arr['s'] = input_check($arr['s']);
    }
    if (isset($arr['db-s-where'])) {
        $arr['db-s-where'] = input_check($arr['db-s-where']);
    }
