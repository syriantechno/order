    ...
    if (isset(til()->page)) {
        ...
        <script>
            $(document).ready(function () {
                ...
                if (!isset($arr['title'])) {
                    $arr['title'] = 'نموذج اضافة موظف';
                }

                if(isset($arr['title'])) {
                if (strlen($arr['title']) < 1) {
                    $arr['title'] = 'المستقبل!';
                }
                ?>
                $('h3.page-title').html('<?php echo $arr["title"]; ?>');
                document.title = '<?php echo $arr["title"]; if ($arr["title"] != 'Power ERP!') {
                    echo ' | المستقبل!';
                } ?>';
                <?php
                }
...
function pagination($count)
{
    ...
    <nav aria-label="Page navigation" class="text-center">
        <ul class="pagination">
                ...
                <li class="<?php if ($btn_after == '#'): ?>disabled<?php endif; ?>">
                    <a href="<?php echo $btn_after; ?>" aria-label="Previous">
                        <span aria-hidden="true"><i class="fa fa-angle-double-left" aria-hidden="true"></i> <span
                                    class="hidden-xs">الصفحة الأولى</span></span>
                    </a>
                </li>
            <?php endif; ?>
            ...
            <?php if (($page - 3) > 0 AND $show_left_right_limit >= 3): ?>
                <li>
                    <a href="<?php echo get_set_url_parameters(array('add' => array('page' => $page - 3))); ?>"><?php echo $page - 3; ?></a>
                </li> <?php endif; ?>
            <?php if (($page - 2) > 0 AND $show_left_right_limit >= 2): ?>
                <li>
                    <a href="<?php echo get_set_url_parameters(array('add' => array('page' => $page - 2))); ?>"><?php echo $page - 2; ?></a>
                </li> <?php endif; ?>
            <?php if (($page - 1) > 0): ?>
                <li>
                    <a href="<?php echo get_set_url_parameters(array('add' => array('page' => $page - 1))); ?>"><?php echo $page - 1; ?></a>
                </li> <?php endif; ?>
            ...
            <?php if (($page + 2) <= $paged AND $show_left_right_limit >= 2): ?>
                <li>
                    <a href="<?php echo get_set_url_parameters(array('add' => array('page' => $page + 2))); ?>"><?php echo $page + 2; ?></a>
                </li> <?php endif; ?>
            <?php if (($page + 3) <= $paged AND $show_left_right_limit >= 3): ?>
                <li>
                    <a href="<?php echo get_set_url_parameters(array('add' => array('page' => $page + 3))); ?>"><?php echo $page + 3; ?></a>
                </li> <?php endif; ?>
            ...
            <?php if ($btn_before != '#'): ?>
                <li class="<?php if ($btn_before == '#'): ?>disabled<?php endif; ?>">
                    <a href="<?php echo $btn_before; ?>" aria-label="Previous">
                        <span aria-hidden="true"><span class="hidden-xs">الصفحة الأخيرة</span> <i
...
function search_form_for_panel($arr = array())
{
    ...
    ?>
    <form name="form_panel_search_<?php echo $arr['s_name']; ?>" id="form_panel_search_<?php echo $arr['s_name']; ?>"
          action="" method="GET" class="panel-search">
        <div class="input-group">
            <div class="input-group-btn">
                <?php if (isset($arr['db-s-where'])): ?>
                    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"
                            aria-expanded="false"><span class="btn-text">عام</span> <span class="caret"></span></button>
                    <ul class="dropdown-menu dropdown-menu-left" role="menu">
                        ...
                        <?php foreach ($arr['db-s-where'] as $sWhere): ?>
                            <li><a href="#" data-click="db-search"
                                   data-db-table-name="<?php echo $sWhere['val']; ?>"><?php echo $sWhere['name']; ?></a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
            </div><!-- /btn-group -->
            <input type="search" name="s" id="panel_s_<?php echo $arr['s_name']; ?>" class="form-control"
                   placeholder="اكتب ماتريد البحث عنه هنا" value="<?php echo @$_GET['s']; ?>">
            <div class="input-group-btn">
                <button type="submit" class="btn btn-default panel-search-btn"><i class="fa fa-search"></i></span>
                </button>
            </div>
...
{
    if (!isset($args['codeType'])) {
        $args['codeType'] = 'Code128';
    }
    if (!isset($args['print'])) {
        $args['print'] = 'false';
    }
                    ...
                    <div class="modal-header">
                        <?php if ($args['x']): ?>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button><?php endif; ?>
                        <?php if (isset($args['title'])): ?><h4 class="modal-title"
                                                                id="myModalLabel"><?php echo $args['title']; ?></h4><?php endif; ?>
                    </div>
                <?php endif; ?>
                <?php if ($args['body']): ?>
