...
function chartjs($args)
{
    ...
    // chart type
    if (!isset($args['id'])) {
        $args['id'] = 'tilChart-' . rand(11111111, 99999999);
    }
    if (!isset($args['backgroundColor'])) {
        $args['backgroundColor'] = true;
    }
    if (!isset($args['borderColor'])) {
        $args['borderColor'] = true;
    }
    ...
    if ($args['backgroundColor'] == true) {
        ...
        $args['backgroundColor'][] = 'rgba(0, 255, 255, 0.5)';
    ...
    if (count($args['chart']['data']['datasets']) == 1) {
        ...
        foreach ($args['chart']['data']['datasets'] as $key => $value) {
            if (!isset($value['backgroundColor'])) {
                $args['chart']['data']['datasets'][$key]['backgroundColor'] = $args['backgroundColor'][$key];
            }
            if (!isset($value['borderColor'])) {
                $args['chart']['data']['datasets'][$key]['borderColor'] = $args['borderColor'][$key];
            }
        }
    }
    ...
    <canvas id="<?php echo $args['id']; ?>" width="<?php echo $args['width']; ?>"
            height="<?php echo $args['height']; ?>"></canvas>
    <script>
