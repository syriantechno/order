<?php if (isset($_GET['session_id']) AND isset($_GET['get_message']) AND isset($_GET['query_message']) AND isset($_GET['type'])) {
    session_id($_GET['session_id']);
} else {
    exit;
} ?>
<?php include('../../ultra.php'); ?>
<?php if (!is_login() AND empty($_GET['get_message']) AND empty($_GET['query_message']) AND empty($_GET['type'])) {
    exit;
}
if ($_GET['get_message'] == "old" AND isset($_GET['list_old_message'])) {
    if ($get_message = get_message($_GET['list_old_message'], array('type' => $_GET['type']))) :
        if ($_GET['type'] == 'mess-reply') {
            $or_type = 'message';
        } elseif ($_GET['type'] == 'task-reply') {
            $or_type = 'task';
        }
        ...
        if ($messages = array_reverse(get_messages(array('query' => $get_query)))): ?>
            <?php foreach ($messages as $message): ?>
                <div class="message-elem  message-<?php echo $message->id; ?> <?php if ($message->rec_u_id == get_active_user('id')) {
                    echo 'get';
                } else {
                    echo 'send';
    ...
    if ($messages = get_messages(array('query' => $get_query))): ?>
        <?php foreach ($messages as $message): ?>
            <?php if ($message->rec_u_id == get_active_user('id')) {
                db()->query("UPDATE " . dbname('messages') . " SET read_it='1' WHERE id='" . $message->top_id . "' ");
            } ?>
            <div class="message-elem  message-<?php echo $message->id; ?> <?php if ($message->sen_u_id == get_active_user('id')) {
                echo 'send';
            } else {
                echo 'get';
            } ?>" id="<?php echo $message->id; ?>" title="<?php echo $message->title; ?>"
