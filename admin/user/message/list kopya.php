...
if (isset($_GET['outbox'])) {
    $messagebox = 'outbox';
    add_page_info('title', 'صندوق الرسائل - مفتوح');
}
if (isset($_GET['trash'])) {
    $messagebox = 'trash';
    add_page_info('title', 'صندوق الرسائل - محذوف');
}
                    ...
                    <?php if (isset($messages)): ?>
                        <table class="table table-hover table-condensed table-striped dataTable">
                            ...
                            <?php foreach ($messages as $list): ?>
                                <tr>
                                    <td width="10">
                                        <a href="?id=<?php echo $list->id; ?>&move=null&<?php if ($list->inbox_u_id == get_active_user('id')) {
                                            echo 'inbox';
                                        } else {
                                            echo 'outbox';
                                        } ?>" class="btn btn-default btn-xs" data-toggle="tooltip"
                                           title="أخرجه من سلة المهملات"><i class="fa fa-reply text-warning"></i></a>
                                    </td>
                                    <td width="140"><?php echo mb_substr(get_user_info($list->sen_u_id, 'name'), 0, 1); ?>
                                        . <?php echo get_user_info($list->sen_u_id, 'surname'); ?></td>
                                    <td width="140"><?php echo mb_substr(get_user_info($list->rec_u_id, 'name'), 0, 1); ?>
                                        . <?php echo get_user_info($list->rec_u_id, 'surname'); ?></td>
                                    <td>
                                        <a href="detail.php?id=<?php echo $list->id; ?>"
                                           class="<?php if (!$list->read_it and $list->inbox_u_id == get_active_user('id')): ?>bold<?php endif; ?>">
                ...
                <?php else: ?>
                    <?php if ($messages): ?>
                        <table class="table table-hover table-condensed table-striped dataTable">
                            ...
                            <tbody>
                            <?php foreach ($messages as $list): ?>
                                <tr class="<?php if (!$list->read_it and $list->inbox_u_id == get_active_user('id')): ?>bold<?php endif; ?>"
