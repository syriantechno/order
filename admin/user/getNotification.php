...
<?php if (!is_login() AND empty($_GET['box']) AND empty($_GET['query_type'])) {
    exit;
} ?>
<?php
...
if ($_GET['query_type'] == 'count') {
    ...
    if ($_GET['box'] == 'message') : ?>
        ...
        <li class="message-footer"><a href="<?php site_url('admin/user/' . $_GET['box'] . '/list.php'); ?>">عرض الكل</a>
        </li>
    ...
    if ($_GET['box'] == 'notification') : ?>
        <li class="message-header">
            ...
            <span class="pull-right">
					<button onclick="notification_all_readit_button(this, parent(this, '.dropdown-message-list'))"
                            style="border: 0; background: none; outline: none; padding-right: 15px;"><i
            ...
            <?php foreach ($messages as $message): ?>
                <li class="message-list notification-<?php echo $message->id; ?> <?php if (!$message->read_it): ?>bold<?php endif; ?>">
                    <a href="<?php echo $message->message; ?>"
                       onclick="if (event.target.tagName != 'I') { set_notification(this, {'id': '<?php echo $message->id; ?>', 'read_it': '1', 'status': '1'}); } else { return false; } ">
                        <div class="row no-space">
                            <div class="col-md-2 col-xs-2">
                                <div class="message-avatar">
                                    <i class="<?php if (empty($message->writing)) {
                                        echo 'fa fa-globe';
                                    } else {
		            ...
		            <span class="message-time">
									...
									<button
                                            ...
                                            data-wenk="<?php if ($message->read_it) {
                                                echo 'okunmadı';
                                            } else {
                                                echo 'okundu';
                                            } ?>"
									...
									<button style="border: 0; background: none !important; outline: none;"
                                            data-wenk="Gizle" data-wenk-pos="top"
                                            onclick="set_notification(this, {'id': '<?php echo $message->id; ?>', 'read_it': '1', 'status': '0'}); return false;"
                                            onchange="notification_delete_button(this)">
        ...
        <?php if (!til_is_mobile()): ?>
            <li class="message-footer"><a href="<?php site_url('admin/user/' . $_GET['box'] . '/list.php'); ?>">عرض
                الكل</a></li><?php endif; ?>
    <?php endif;
} else {
    exit;
