...
if (!$user = get_user($_GET['id'])) {
     add_console_log('Kullanıcı hesabı bulunamadı.', __FUNCTION__);
    return false;
}
@$user_meta = get_user_meta($user->id);
set_staff_salary($user->id);
if (!$account = get_account($user->account_id)) {
    add_console_log('لم يتم العثور على بطاقة حساب للمستخدم.', __FUNCTION__);
}
...
<div class="print-page" size="A4">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <div class="text-right">
                <small class="text-muted"><?php echo til_get_date(true, 'datetime'); ?></small>
            </div>
                        ...
                        <?php if ($q_form_items->num_rows): ?>
                            <?php while ($item = $q_form_items->fetch_object()): ?>
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <small class="text-muted"><?php echo get_set_money($item->price, true); ?></small>
                                    </td>
                                    <td class="text-right">
                                        <small class="text-muted"><?php echo get_set_money($item->total, true); ?></small>
                                    </td>
                                    <td></td>
                                    <td></td>
                                </tr>
    ...
    <div class="print-footer print-footer-right text-center">
        <small class="text-muted">هذه الوثيقة <?php echo get_active_user('display_name'); ?>
            بواسطة, <?php echo til_get_date(true, 'datetime'); ?> تم الإنشاء.
        </small>
        <br/>
    </div>
